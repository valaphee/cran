 ...
 */
@NodeImpl("virtual")
object AbsoluteVirtual : NodeVirtual {
    override fun initialize(node: Node, virtual: Virtual) = if (node is Absolute) {
        val `in` = virtual.dataPath(node.`in`)
        val out = virtual.dataPath(node.out)

        out.set { vectorOp(`in`.get(), { it.abs() }, { it.abs() }, { it.abs() }, virtual.objectMapper) }
