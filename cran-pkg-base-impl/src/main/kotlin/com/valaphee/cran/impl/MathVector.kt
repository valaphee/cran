...
object MathVector : Implementation {
    override fun initialize(node: Node, scope: Scope) = when (node) {
        is UnaryOperation -> {
            ...
            when (node) {
                is Absolute -> {
                    out.set { unaryOp(`in`.get(), { it.abs() }, { it.abs() }, { it.abs() }, scope.objectMapper) }
        ...
        is BinaryOperation -> {
            ...
            when (node) {
                is Add -> {
                    out.set { binaryOp(in1.get(), in2.get(), { a, b -> a.add(b) }, { a, b -> a.add(b) }, { a, b -> a.add(b) }, scope.objectMapper) }

                    true
                }
                is Subtract -> {
                    out.set { binaryOp(in1.get(), in2.get(), { a, b -> a.sub(b) }, { a, b -> a.sub(b) }, { a, b -> a.sub(b) }, scope.objectMapper) }
